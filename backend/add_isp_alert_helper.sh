#!/usr/bin/env bash
#
# add_isp_alert_helper.sh
#  A) Add utils/ispAlert.js — one place that notifies the ISP account owner by SMS and records
#     the event, with edge-triggering so a flapping link cannot send a message every poll.
#     Nothing is wired to it yet: this step is additive and cannot change behaviour.
#  B) Dump the exact transition points in mwan-applier.js so the calls go in the right place.
#
set -uo pipefail
...
say "A) Create utils/ispAlert.js"
sudo tee "$BE/utils/ispAlert.js" > /dev/null <<'JS'
...
JS
sudo chown www-data:www-data "$BE/utils/ispAlert.js"
sudo -u www-data node --check "$BE/utils/ispAlert.js" && echo "   ispAlert.js syntax OK"

say "B1) Where does mwan-applier record events / status changes?"
sudo grep -nE "nas_events|current_status|wan_quality|logEvent|INSERT INTO" "$BE/utils/mwan-applier.js" | sed 's/^/   /'

say "B2) The failover decision inside monitorDevice"
sudo grep -nE "setPoolMembership|poolVerdict|_distance|default-route-distance|primary|backup|degrad|recover|trip" "$BE/utils/mwan-applier.js" | sed -n '1,40p' | sed 's/^/   /'

say "B3) getLinkStatus — what the device page already shows"
L=$(sudo grep -n "async function getLinkStatus" "$BE/utils/mwan-applier.js" | cut -d: -f1)
[ -n "$L" ] && sudo sed -n "${L},$((L+40))p" "$BE/utils/mwan-applier.js" | nl -ba -v"$L" | sed 's/^/   /'

say "B4) Where wan_quality events are written (the transition points)"
for LN in $(sudo grep -n "wan_quality" "$BE/utils/mwan-applier.js" | cut -d: -f1); do
  echo "   --- around line $LN ---"
  sudo sed -n "$((LN-12)),$((LN+8))p" "$BE/utils/mwan-applier.js" | nl -ba -v"$((LN-12))" | sed 's/^/     /'
done

say "B5) Current link state in the database"
q -c "SELECT n.name AS router, w.position, w.name, w.interface, w.role, w.current_status, w.last_rtt, w.last_checked_at
      FROM nas_wan_links w JOIN nas_devices n ON n.id=w.nas_id ORDER BY w.position;"

say "B6) Is monitorAll scheduled?"
sudo grep -n "monitorAll\|mwan" "$BE/utils/cron.js" | sed 's/^/   /'
