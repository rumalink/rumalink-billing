...
async function stkQueryAndUpdate(paymentId) {
  ...
  try {
    ...
    } else if (q.data.ResultCode && q.data.ResultCode !== '1032') {
      // 1032 = still pending/cancelled by user prompt open; anything else final-failed
      await query("UPDATE payments SET status='failed', failure_reason=$1 WHERE id=$2::uuid AND status='pending'",
        [String(q.data.ResultDesc || 'Failed').substring(0,200), paymentId]).catch(()=>{});
    }
  } catch (e) {
...
router.get('/public-status/:paymentId', async (req, res) => {
  try {
    ...
    if (r.rows[0].status === 'pending') {
      ...
      r = await query('SELECT id, status, transaction_id, amount, failure_reason FROM payments WHERE id=$1::uuid', [req.params.paymentId]);
    }
    res.json({
