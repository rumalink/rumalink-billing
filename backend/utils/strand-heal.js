...
async function pass(onlyPaymentId) { /* RL_HEAL_ONE: when given, heal just that payment (inline, fast) */
  ...
  // ---- RL_MAC_BACKFILL: learn each voucher's device MAC from RADIUS accounting ----
  // Captive auto-login navigates straight to the router's /login, so the captive.js paths that set
  // used_by_mac never run: the column stayed NULL, the MAC sweep below had nothing to write, and
  // returning customers got Access-Reject on mac-auth. The router already reports the real device as
  // Calling-Station-Id on every accounting start, so take it from there — authoritative, and no
  // change to the login flow is needed.
  try {
    const macReB = '^([0-9A-Fa-f]{2}:){5}[0-9A-Fa-f]{2}' + String.fromCharCode(36);
    const bf = await query(
      "UPDATE hotspot_vouchers v SET used_by_mac = UPPER(ra.callingstationid), updated_at = NOW() " +
      "FROM ( SELECT DISTINCT ON (username) username, callingstationid, acctstarttime FROM radacct " +
      "       WHERE callingstationid ~ '" + macReB + "' ORDER BY username, acctstarttime DESC ) ra " +
      "WHERE v.status='active' AND v.expires_at > NOW() AND (v.is_tv IS NOT TRUE) " +
      "  AND ra.username = v.code || '@' || LOWER(SUBSTRING(REPLACE(v.isp_id::text,'-','') FROM 1 FOR 8)) " +
      "  AND (v.used_by_mac IS NULL OR UPPER(v.used_by_mac) <> UPPER(ra.callingstationid))");
    if (bf && bf.rowCount) logger.info('[strand-heal] RL_MAC_BACKFILL: learned device MAC for ' + bf.rowCount + ' voucher(s)');
  } catch (e) { logger.warn('[strand-heal] mac backfill: ' + e.message); }
