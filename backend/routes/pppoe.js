...
router.post('/packages', async (req, res, next) => {
  ...
  try {
    ...
    `, [req.user.ispId, name, description, price, billing_cycle || 'monthly', bandwidth_down_mbps, bandwidth_up_mbps, data_limit_gb, burst_limit_mbps, mikrotik_profile, address_pool,
        /* RL_PPPOE_VIS_PARAM: $12 was declared with no value. Default true — offered unless hidden. */
        (req.body.visible_in_portal === false ? false : true)]);
  ...
  try {
    const result = await query(`
      UPDATE pppoe_packages SET name=$1, description=$2, price=$3, billing_cycle=$4,
        ...
        mikrotik_profile=$8, address_pool=$9, is_active=$10,
        /* RL_PPPOE_VIS_PARAM: an earlier edit reused $11, which is the package id — it would have
           tried to store a UUID in a boolean. $13 is its own slot; COALESCE leaves the value alone
           when the caller omits it, so editing a price cannot republish a hidden package. */
    ...
    `, [name, description, price, billing_cycle, bandwidth_down_mbps, bandwidth_up_mbps, data_limit_gb, mikrotik_profile, address_pool, is_active, req.params.id, req.user.ispId,
        (req.body.visible_in_portal === undefined ? null : !!req.body.visible_in_portal)]);
