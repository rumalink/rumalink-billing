  ...
  try {
    const result = await query(`
      SELECT hv.id, hv.code, hv.status, hv.used_at, hv.expires_at,
             ...
             n.name as device_name, hv.is_test
...
router.post('/generate', async (req, res, next) => {
  ...
  try {
    ...
    for (let i = 0; i < quantity; i++) {
      const code = await generateIspUsername(req.user.ispId);
      usedCodes.add(code);
...
// Update a voucher (expires_at, status, buyer_phone)
router.put('/:id', async (req, res, next) => {
  try {
    const { expires_at, status, buyer_phone } = req.body;
    const updates = [];
    const params = [];
    let i = 1;
    if (expires_at !== undefined) { updates.push(`expires_at = $${i++}::timestamptz`); params.push(expires_at || null); }
    if (status !== undefined) { updates.push(`status = $${i++}`); params.push(status); }
    if (buyer_phone !== undefined) { updates.push(`buyer_phone = $${i++}`); params.push(buyer_phone); }
    if (!updates.length) return res.status(400).json({ error: 'No fields to update' });
    updates.push('updated_at = NOW()');
    params.push(req.params.id, req.user.ispId);
    const r = await query(
      `UPDATE hotspot_vouchers SET ${updates.join(', ')} WHERE id = $${i++}::uuid AND isp_id = $${i++}::uuid RETURNING *`,
      params
    );
    if (!r.rows[0]) return res.status(404).json({ error: 'Voucher not found' });
