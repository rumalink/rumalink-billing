...
router.get('/:ispId', async (req, res, next) => {
  try {
    ...
    const packages = await query(
      `SELECT id, name, description, price, duration_hours, bandwidth_down_mbps, bandwidth_up_mbps, data_limit_mb
       FROM hotspot_packages WHERE isp_id=$1 AND is_active=true ORDER BY price ASC`,
      [req.params.ispId]
    );
    const payMethods = await query(
      `SELECT id, method_type, label, till_number, paybill_number, account_reference, bank_name, account_name, account_number
       FROM isp_payment_methods WHERE isp_id=$1 AND is_active=true`,
      [req.params.ispId]
    );
...
router.post('/:ispId/pay', async (req, res, next) => {
  ...
  try {
    ...
    // Get ISP's active M-Pesa STK method (fallback to admin creds)
    ...
    if (!mpesaCreds.consumer_key || !mpesaCreds.consumer_secret || !mpesaCreds.shortcode || !mpesaCreds.passkey) {
      return res.status(400).json({ error: 'M-Pesa not configured for this portal. Please contact support.' });
    }
