...
router.post('/', async (req, res, next) => {
  const {
    ...
    notif_enabled, notif_title, notif_message, notif_severity, notif_start, notif_end, /* RL_NOTIF_BANNER */
    media_enabled, media_type, media_video_url, media_images, media_interval, /* RL_MEDIA_BANNER */
    media_fit /* RL_MEDIA_FIT */
  } = req.body;
  ...
  try {
    const result = await query(`
      INSERT INTO isp_captive_portal (
        ...
        notif_enabled, notif_title, notif_message, notif_severity, notif_start, notif_end,
        media_enabled, media_type, media_video_url, media_images, media_interval, media_fit
      ...
      ON CONFLICT (isp_id) DO UPDATE SET
        ...
        notif_enabled=COALESCE($15, isp_captive_portal.notif_enabled),
        notif_title=COALESCE($16, isp_captive_portal.notif_title),
        notif_message=COALESCE($17, isp_captive_portal.notif_message),
        notif_severity=COALESCE($18, isp_captive_portal.notif_severity),
        notif_start=$19,
        notif_end=$20,
        media_enabled=COALESCE($21, isp_captive_portal.media_enabled),
        media_type=COALESCE($22, isp_captive_portal.media_type),
        media_video_url=$23,
        media_images=COALESCE($24, isp_captive_portal.media_images),
        media_interval=COALESCE($25, isp_captive_portal.media_interval),
        media_fit=COALESCE($26, isp_captive_portal.media_fit),
        updated_at=NOW()
      RETURNING *
    `, [req.user.ispId, template, primary_color, secondary_color, logo_url,
        ...
        notif_enabled === true, notif_title || null, notif_message || null,
        notif_severity || 'info', notif_start || null, notif_end || null,
        media_enabled === true, media_type || 'carousel', media_video_url || null,
        media_images ? JSON.stringify(media_images) : null,
        media_interval || 4,
        media_fit || 'contain']);
