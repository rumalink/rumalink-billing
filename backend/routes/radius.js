...
// Persist hotspot accounting into radacct (mirrors how PPPoE persists via FreeRADIUS).
// Hotspot authenticates as code@ispShort but accounts as MAC, so radacct bytes never updated.
// This resolves MAC -> active voucher -> realmed username and upserts the byte counters.
async function writeHotspotRadacct({ sid, nMac, dl, ul, dur, nasIp, ip, cause, isStop }) {
  try {
    const v = await query(
      "SELECT code, isp_id FROM hotspot_vouchers WHERE used_by_mac=$1 ORDER BY used_at DESC NULLS LAST LIMIT 1",
      [nMac]
    );
    if (!v.rows[0]) return;
    const ispShort = String(v.rows[0].isp_id).replace(/-/g, '').slice(0, 8).toLowerCase();
    const realmedUser = v.rows[0].code + '@' + ispShort;
    const acctSid = sid || ('hs_' + nMac.replace(/[:\-]/g, '') + '_' + Date.now());
    // Try update by session id first (the row FreeRADIUS created on auth).
    const upd = await query(
      "UPDATE radacct SET acctinputoctets=$1, acctoutputoctets=$2, acctsessiontime=$3, acctupdatetime=NOW()" +
      (isStop ? ", acctstoptime=NOW(), acctterminatecause=$6" : "") +
      " WHERE acctsessionid=$4 AND username=$5",
      isStop ? [ul, dl, dur, acctSid, realmedUser, cause || 'User-Request'] : [ul, dl, dur, acctSid, realmedUser]
    );
    if (upd.rowCount === 0) {
      // No row to update (auth row absent or different sid) — insert a fresh accounting row.
      await query(
        "INSERT INTO radacct (acctsessionid, acctuniqueid, username, nasipaddress, acctstarttime, acctupdatetime, " +
        " acctstoptime, acctsessiontime, acctinputoctets, acctoutputoctets, callingstationid, framedipaddress, acctterminatecause, realm) " +
        "VALUES ($1,$2,$3,$4::inet, NOW(), NOW(), $5, $6, $7, $8, $9, NULLIF($10,'')::inet, $11, $12) " +
        "ON CONFLICT DO NOTHING",
        [acctSid, (acctSid + '_' + realmedUser).slice(0,32), realmedUser, nasIp || '127.0.0.1',
         isStop ? new Date() : null, dur, ul, dl, nMac, ip || '', isStop ? (cause||'User-Request') : null, ispShort]
      );
    }
  } catch (e) { try { logger.warn('writeHotspotRadacct: ' + e.message); } catch(_){} }
}
...
router.post('/accounting', async (req, res) => {
  ...
  try {
    if (st === 'Start') {
      ...
      if (isMac) { await writeHotspotRadacct({ sid, nMac: normalizeMac(user), dl, ul, dur, nasIp, ip, cause, isStop: true }); }
      ...
      if (isMac) { await writeHotspotRadacct({ sid, nMac: normalizeMac(user), dl, ul, dur, nasIp, ip, cause, isStop: false }); }
      if (isMac) {
        const nMac = normalizeMac(user);
